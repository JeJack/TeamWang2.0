            ...
            <div class="row">
                <%
                    ...
                    for (int j =0;j<3&&i<userCollectionArrayList.size();i++){
                        if ("building".equals(userCollectionArrayList.get(i).getCollectionCategory())){
                            BuildingDao buildingDao = new BuildingDao();
                            Building building = buildingDao.getBuildingById(userCollectionArrayList.get(i).getCollectionCategoryId());
                            BuildingImageDao buildingImageDao = new BuildingImageDao();
                            ArrayList<BuildingImage> buildingImages = buildingImageDao.getAllBuildingImageNameByBuildingId(building.getBuildingId());
                %>
                <div class="col-sm-6 col-md-4 ser">
                    <div class="thumbnail">
                        <div class="caption">
                            <p><%=building.getBuildingName()%></p>
                        </div>
                        <div class="caption">
                            <a href="../building/buildingDetails.jsp?id=<%=building.getBuildingId()%>">
                                <img src="<%=buildingImages.get(0).getBuildingImageName()%>"width="318px" height="300" alt="图片无法显示"/></a>
                        </div>
                        <div class="caption">
                            <p><%=building.getBuildingCharacteristic()%></p>
                        </div>
                    </div>
                </div>
                <%
